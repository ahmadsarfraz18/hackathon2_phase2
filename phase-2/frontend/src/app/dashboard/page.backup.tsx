...
export default function DashboardPage() {
  const { user, loading, isInitialized, isAuthenticated, logout, refreshSession } = useAuth();
  ...
  // Import task API functions locally to avoid potential circular dependencies
  const taskApi = {
    getTasks: async (): Promise<Task[]> => {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8000'}/tasks`, {
        headers: {
    ...
    updateTask: async (taskId: string, taskData: Partial<Task>): Promise<Task> => {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8000'}/tasks/${taskId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  }, [isInitialized, isAuthenticated, user]); // Removed logout from dependencies to prevent infinite re-renders
